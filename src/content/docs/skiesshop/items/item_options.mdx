---
title: Item Options
description: Guide to configure generic item's appearance and data in the SkiesShop mod
---
...
### Actions
:::warning
This option is not always available depending on the context of the item. For example, in a Shop Entry's `"display": {}` configuration, the actions setting is not available, but it is for the `"items"` section of a Confirmation Menu.
:::
A list of Actions that are executed when the item is clicked on. See [Actions](/skiesshop/items/actions/) for more information.
```json
"actions": {
  "action_1": {
