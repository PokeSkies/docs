...
description: Configuration options for data storage in Actionitems
---
...
- `JSON` - <Badge text="Danger" variant="danger" /> Stores data in a JSON file located at `config/actionitems/storage.json`. Not recommended for production servers due to performance.
- `SQLITE` - Stores data in a SQLite database located at `config/actionitems/storage.db`.
