...
Components in configs are fundamentally the same, but formatting-wise have some differences. For example, if we wanted to convert this example give command with the components to a ActionItems item configuration, it would look something like this:
```json
{
  "display": {
